...
# IoT Manager for ESP32 supporting WiFi config and OTA
# GNU GENERAL PUBLIC LICENSE license; Copyright (c) 2023 Oliver Fueckert
...
class IotManager:
    def __init__(self, title="IoT Manager", ota_url="", use_css=True, debug=False, config={}):
        if debug:
        ...
        # Init config
        self.config_load()
        self.config_add(default_config)
        self.config_add(config, override=True)
        ...
        # Init OTA
        self.ota = False
        if self.ota_partiton_available():
            log.debug("Init OTA")

            self.ota_boot_successful()

            self.config_add({Config.OTA_URL: ota_url})
            self.ota = True
    ...
    def connect(self, ssid="", pwd=""):
        ...
        # AP Mode if no SSID
        if not ssid:
            log.info(f"Starting in AP Mode: {ap_if.ifconfig()}")
            ap_if.active(True)
            start = time.ticks_ms()
        ...
        # Try to connect as client with SSID
        if ssid:
            sta_if.active(True)
            ...
            while not sta_if.isconnected():
                ...
                if time.ticks_diff(time.ticks_ms(), start) > 10000:
                    log.error(f"WIFI connection timeout to SSID {ssid}: {sta_if.status()}")
            ...
            log.info(f"WLAN (STA) connected: {sta_if.ifconfig()}")
            ap_if.active(False)
            return sta_if
    ...
    async def ota_update(self, writer, ota_url):
        ...
        try:
            log.info(f"Connecting to OTA server: {host}:{port}")
